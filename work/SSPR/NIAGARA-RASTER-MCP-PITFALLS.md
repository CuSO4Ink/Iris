...
相关文档：
- 当前算法规格：`ANISOTROPIC-GAUSSIAN-SPLAT-SPEC.md`
- 当前性能规格：`M3-PERF-OPTIMIZATION-SPEC-20260731.md`
- 当前解析核规格：`ANALYTIC-GAUSSIAN-SPLAT-SPEC-20260804.md`
- 旧 Grid/RT 调试记录：`archive/2026-08-05-cleanup/docs/RT-WRITE-DEBUG.md`
- 历史总体规格：`archive/2026-08-05-cleanup/docs/WISPY-FLUID-SPEC.md`
- 时间线：`LOG.md`
...
### 4.6 屏幕空间运行 Gate 必须固定相机并先证明投影有效

SIE 会沿用当时的 Level Editor 视口相机。System/Stage/DI 全部正常时，只要相机没有覆盖粒子系统，屏幕空间注册和 Gather 就会得到合法的全零结果。本项目第一次 P0 Gate 因此把正常链误判为失败。

正式 Gate 必须：

1. 记录并设置确定的测试相机 Location/Rotation；
2. 用粒子属性或 SimCache 抽样确认 `ScreenUV` 有限且落在 `0..1`、ViewDepth 为正；
3. 先让未修改控制组在同一 World、相机、分辨率和负载下得到非零生产 RT；
4. 再读取 NeighborQuery marker 或候选生产 RT；
5. 候选为零而控制组也为零时，只能判定 harness 无效，不能判定候选失败；
6. 结束后恢复原相机，注意 `unreal.Rotator` 使用显式 `pitch=/yaw=/roll=`，不要依赖位置参数顺序。

“Emitter Active”“粒子数非零”和 verbose Stage 日志都不能代替这一步，因为它们不证明当前视角下有有效屏幕覆盖。

### 4.7 ParticleRead 的 `Other/None` 可在未使用路径中潜伏，切换 gather 后才致命

RecordPoint 的 RasterizationGrid3D 生产路径虽然携带 Emitter 级 ParticleRead DI，但主 gather 并不调用它，因此 `Emitter.P0_ParticleRead={bindingMode:Other, emitterName:None}` 可以长期不阻断 Raster 输出。把 Stage B 替换为 NeighborQuery 后，`GetParticleNeighbor` 返回的索引必须再交给 ParticleRead 取 `Particles.Position/ScreenUV/Depth`；同一个旧绑定会立刻产生：

```text
Source emitter 'None' not found
GPU particle read could not find GPU context for '...None'
```

规则：

- DI 存在、模块连线和 HLSL 编译通过，都不能证明 ParticleRead 的 emitter binding 有效；
- current-emitter gather 必须 typed 读回确认 `bindingMode=Self`；跨 emitter Reader 则必须是存在且精确匹配的 `Other/<EmitterName>`，禁止 `Other/None`；
- 变更绑定前先停 PIE，只改隔离候选，保存后再做一次独立 typed 读回；
- 运行 Gate 同时检查日志与双 RT，只有 `Source emitter None` 消失且跨帧 RT 非零/finite 才算修复闭环；
- 不要为了消除另一个 `AddParticle should be in a particle-writing stage` 警告就移动 Stage A。该警告需要运行证据评估；若当前 Stage 已稳定跨帧输出，移动执行位置属于新的架构改动，必须另开候选。
...
项目探针采用分条读取，并按实际实现传参。正确的 2048×256 strip 是：

```python
read_render_target_raw_pixel_area(
    world, target, 0, y0, 2048, 256, False
)
```

第五、第六个区域参数是 `Width/Height`，不是右下角 `MaxX/MaxY`；最后一个参数继续按 5.6 使用 `False`。例如第二条仍应传 `(0, 256, 2048, 256)`，不能传 `(0, 256, 2048, 512)`。复制脚本时不要自行改回常规矩形 Max 坐标语义。
...
### 5.6 绝对 marker/计数读回必须关闭归一化

`unreal.RenderingLibrary.read_render_target_raw(world, target, True)` 的第三个参数会归一化读回。P0 的绝对 marker 原本为 `1..55`，开启后被压成 `0..1`，从而被错误分类为“只执行但 CellCount=0”。

规则：

- 诊断 marker、计数、深度、密度和值域 Gate 使用 `False`；
- `True` 只用于明确需要归一化预览的场景；
- 结果中记录 `readbackNormalized`，避免后续日志失去口径；
- 若预期最小值本应为 1、实际却出现 `0.0108` 一类比例值，先检查归一化参数，不要改 HLSL。

### 5.7 2048² RT 全量像素必须在 UE 内聚合，禁止跨 MCP 返回像素数组

一次 2048² RGBA 读回有 4,194,304 个颜色元素；Python wrapper、JSON 字符串和 PowerShell 对象化会把显存中的紧凑数据放大到数 GB 甚至数十 GB。把两张 Main/Aux RT 的像素逐项打印或 `ConvertFrom-Json`，会重现 Gateway PowerShell 占满内存、UE 看似无响应的事故。

正式做法：

1. 用隔离的 `python.execute` 在 UE 内按 2048×256 分成 8 条读取，每次只持有当前 Main/Aux strip；
2. 在 UE Python 内直接累计 min/max/nonzero/nonfinite/histogram/边缘/矩约束；
3. 删除 strip 引用并 `gc.collect()`，只向 MCP 返回聚合 JSON；
4. PowerShell 保持 one-shot，不创建常驻像素数组，不把整张 RT 落成 JSON；
5. SSPR 每次调用显式使用 Gateway 的 1 GiB private-memory guard（`-ProcessGuardMaxPrivateMemoryMB 1024`）与硬超时，超时后先查孤儿 PID/命令行/内存，再决定是否终止。

这条规则同时适用于 SimCache 大数组、纹理导出和任何百万级元素的 Python 返回值。若确实需要图像证据，应让 UE 写工具自有截图文件，只返回路径。
...
5. SSPR 当前 Gateway one-shot/daemon 均必须显式保留 `ProcessGuardMaxPrivateMemoryMB=1024` 与硬请求期限；`doctor.ps1` 也必须把同一上限透传给内部 Gateway 探针。不要为“大 RT 读回”关闭 guard，应改成 UE 内分条聚合。
...
### 6.5 PIE Reinitialize 后 DI clone 后缀不是稳定身份

World Partition + Niagara SIE 中，`reinitialize_system()` 会生成新的 component-local DI 代次。一次实测中 User Variables 的绑定从 RT `_0/_1` 切到 `_2/_3`，随后内存里又出现 `_8/_9`；旧 clone 仍可被 ObjectIterator 看见。只配置“编号最小”或“上次使用”的 DI，会让实际 PIE World 目标继续保持 2048²，而脚本误以为已经切到 256²。

正确流程：

1. Start SIE 后读取当前 PIE NiagaraComponent 的 User Variables，解析 AuxRT/SimRT/Grid 的完整 `refPath`；
2. 配置该代 DI，再 Reinitialize；
3. 重新解析绑定，不复用旧 UObject 或数字后缀；
4. 直接枚举当前 PIE World 拥有的实际 `TextureRenderTarget2D`，尺寸必须与 Gate 分辨率严格一致后才读回；
5. StopPIE 前释放所有 PIE UObject 引用。

### 6.6 外部 Actor 写脏后，下一次 Python 请求可能在脚本前自动保存

当前 `execute_python_code` 入口会在非 PIE 状态下先执行 dirty package 的 headless save，再运行用户脚本。实测中，组件恢复脚本把 World Partition 外部 Actor 留脏；下一次本意只是恢复视口相机的 Python 请求先打印 `InternalPromptForCheckoutAndSave`，并把该外部 Actor 写进 Content，之后才执行相机代码。

因此：

- 测试属性尽量只改 PIE clone，不改编辑器 World 的外部 Actor；
- 如果必须改编辑器组件，调用前先建立精确外部 Actor 备份并把该调用视为潜在保存边界；
- StopPIE 后不要用另一条 Python 请求“先检查脏包再恢复”，检查动作本身可能先保存；
- 优先用不会触发 Python pre-save 的已验证 typed tool 恢复，或在明确无其他脏包时退出 UE、离线恢复唯一目标文件；
- `.umap` 哈希不变不能证明 World Partition 关卡没被改，必须同时核对 `Content/__ExternalActors__/...`。
...
| 屏幕空间 RT 全零但 Stage/粒子都活跃 | 视口相机没有覆盖目标 | 固定测试相机；抽验 ScreenUV/ViewDepth |
| 绝对 marker 只落在 0..1 | raw read 开启了归一化 | `read_render_target_raw(..., False)` |
...
| Reinitialize 后目标又变回 2048² | User DI 切到新 clone 代次 | 重读 User Variables 与实际 PIE World RT |
| 只读/恢复相机的 Python 请求却写了外部 Actor | 前一请求留下 dirty package，Python pre-save 先执行 | 查 `InternalPromptForCheckoutAndSave`；离线恢复精确包 |
...
## 17. `GetSystemCompileState` 超时不能直接判定 Niagara System 损坏

带内嵌 Scratch Pad Simulation Stage 的同 basename Niagara System，可能在 `GetSystemCompileState` 读端点长期等待并触发 MCP 超时。2026-08-17 的对照中，NeighborQuery 候选和一份新鲜、未修改的 RecordPoint 文件副本都能 live load，精确 HLSL/拓扑读取正常，但该端点均超时；因此这类超时首先是接口适配问题，不是资产编译失败证据。

发生后不要反复调用或把网关会话失效描述成 UE 资产损坏。应重新走 route/doctor，只用有界读端点检查 Scratch compile messages、生成 HLSL、Stack 拓扑、Renderer/DI 绑定和日志，再以跨请求的真实运行 RT Gate 收口。只有编译日志、HLSL 或运行输出提供独立失败证据时，才把候选登记为 Compile/Runtime FAIL。

## 18. NeighborQuery `AddParticle` 放在 side-effect-only Simulation Stage 会有运行警告

`NiagaraDataInterfaceNeighborQuery` 的 `AddParticle` 用执行索引分配粒子私有表槽位。引擎明确建议它位于 Particle Update 或其他“写粒子属性”的阶段；即使自定义 Simulation Stage 以粒子为迭代源，只做 DI side effect 而不写粒子数据时，日志仍会出现 `AddParticle should be in a particle-writing stage. Use at your own risk.`。

这个警告不能靠“脚本 UpToDate”消除，也不能因为 HLSL 中存在 `AddParticleWithRadius` 就视为运行通过。若为了保持现有 Stage 拓扑先保留这种写法，候选必须停在静态状态，等真实 SIE 验证 RT 非零、跨帧稳定和画面无成员抖动；若失败，再在隔离版本中把注册迁入 Particle Update/particle-writing Stage，不能静默改动原系统或压掉警告。
