...
from grid_to_7drgs import _aggregate_grid
...
def adaptive_moment_partition(
    ...
    if not 0.0 <= organic_jitter <= 1.0:
        raise ValueError("organic jitter must be in [0, 1]")
    masses = (
    ...
    def make_node(members: np.ndarray) -> tuple:
        ...
        if len(members) > 1:
            ...
            axis = eigenvectors[:, -1]
            confidence = (eigenvalues[-1] - eigenvalues[-2]) / max(
                float(eigenvalues[-1]), 1e-15
            )
            tangent = rng.normal(size=3)
            tangent -= axis * np.dot(tangent, axis)
            tangent_norm = np.linalg.norm(tangent)
            if organic_jitter and tangent_norm > 1e-12:
                angle = (
                    math.radians(60.0)
                    * organic_jitter
                    * (1.0 - confidence)
                ...
                axis = math.cos(angle) * axis + math.sin(angle) * tangent / tangent_norm
    ...
    while len(leaves) < target:
        ...
        balance = 0.5 + organic_jitter * rng.uniform(-0.08, 0.08)
        cut = int(np.searchsorted(cumulative, cumulative[-1] * balance)) + 1
...
def load_grid_blocks(
    path: Path,
    block_size: int,
    spatial_sigma_ratio: float,
    longest_size_cm: float,
    density_scale: float,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, float, np.ndarray]:
    grid = np.load(path, mmap_mode="r").squeeze().astype(np.float32, copy=False)
    if grid.ndim != 3 or np.min(grid) < 0.0 or not np.isfinite(grid).all():
        raise ValueError("grid must be finite, non-negative and 3D")
    voxel_cm = longest_size_cm / max(grid.shape)
    centers_ue, mean_density, _, cholesky_gl = _aggregate_grid(
        grid, block_size, spatial_sigma_ratio
    )
    centered = centers_ue - (np.asarray(grid.shape, np.float64) - 1.0) * 0.5
    centers_m = centered[:, (0, 2, 1)] * np.asarray((1.0, -1.0, -1.0))
    centers_m *= voxel_cm / 100.0
    cholesky_m = cholesky_gl * (voxel_cm / 100.0)
    covariances_m2 = cholesky_m @ np.transpose(cholesky_m, (0, 2, 1))
    ...
    extinction = mass / (
        GAUSSIAN_VOLUME * np.sqrt(np.linalg.det(covariances_m2))
    )
    # Weighted centers can land exactly on a block boundary and round a few
    # ulps below it; keep their integer block identity stable.
    block_coordinates = np.floor((centers_ue + 1e-6) / block_size).astype(np.int32)
    return centers_m, covariances_m2, extinction, voxel_cm, block_coordinates


def merge_dense_blocks(
    centers: np.ndarray,
    covariances: np.ndarray,
    extinction: np.ndarray,
    target: int,
    ...
    if target < 1 or target > len(centers):
        raise ValueError("grid target must be between one and the occupied block count")
    if target == len(centers):
        return centers, covariances, extinction
    masses = extinction * GAUSSIAN_VOLUME * np.sqrt(np.linalg.det(covariances))
    remove = np.argsort(masses, kind="stable")[-(len(centers) - target):]
    keep_mask = np.ones(len(centers), dtype=bool)
    keep_mask[remove] = False
    keep = np.flatnonzero(keep_mask)
    block_shape = block_coordinates.max(axis=0) + 1
    linear = np.ravel_multi_index(block_coordinates.T, tuple(block_shape))
    lookup = np.full(int(np.prod(block_shape)), -1, np.int64)
    lookup[linear[keep]] = np.arange(len(keep))
    offsets = sorted(
        (
            (x, y, z)
            for x in range(-2, 3)
    ...
    nearest = np.empty(len(remove), np.int64)
    for index, coordinate in enumerate(block_coordinates[remove]):
        match = -1
        for offset in offsets:
            neighbor = coordinate + offset
            if np.any(neighbor < 0) or np.any(neighbor >= block_shape):
                continue
            match = lookup[np.ravel_multi_index(neighbor, tuple(block_shape))]
            if match >= 0:
                break
        if match < 0:
            raise RuntimeError("dense block has no retained neighbor within two cells")
        nearest[index] = match
    ...
    np.add.at(merged_mass, nearest, masses[remove])
    np.add.at(first, nearest, masses[remove, None] * centers[remove])
    np.add.at(
        second,
        nearest,
        masses[remove, None, None]
        * (
            covariances[remove]
            + np.einsum("ni,nj->nij", centers[remove], centers[remove])
        ),
    )
    ...
    merged_extinction = merged_mass / (
        GAUSSIAN_VOLUME * np.sqrt(np.linalg.det(merged_covariances))
    )
    return merged_centers, merged_covariances, merged_extinction
...
def build(args: argparse.Namespace) -> dict:
    if args.grid:
        centers, covariances, extinction, source_voxel_cm, block_coordinates = load_grid_blocks(
            args.grid,
            args.grid_block_size,
            args.grid_spatial_sigma_ratio,
            args.longest_size_cm,
            args.density_scale,
        )
        source_path = args.grid
    else:
        ...
        source_path = args.ply
    counts = budgets = starts = None
    if args.grid:
        centers_out, covariances_out, extinction_out = merge_dense_blocks(
            centers, covariances, extinction, args.target_count, block_coordinates
        )
    elif args.partition == "adaptive":
    ...
    payload = {
        ...
        "method": (
            "fixed_grid_block_moments_dense_core_merge"
            if args.grid
            else
            (
                "adaptive_organic_partition_moment_contraction"
                if args.organic_jitter
                else "adaptive_binary_partition_moment_contraction"
            )
...
def _self_check() -> None:
    ...
    organic = adaptive_moment_partition(
        centers,
        covariances,
        extinction,
        2,
    ...
    assert len(organic[0]) == 2
    assert np.isclose(
        before,
        np.sum(
            organic[2]
            * GAUSSIAN_VOLUME
            * np.sqrt(np.linalg.det(organic[1]))
        ),
    )
    grid = np.zeros((4, 4, 4), np.float32)
    grid[:3] = 1.0
    grid_path = Path(__file__).with_name("_grid_budget_self_check.npy")
    try:
        np.save(grid_path, grid)
        blocks = load_grid_blocks(grid_path, 2, 0.5, 4.0, 0.1)
        merged = merge_dense_blocks(
            *blocks[:3], len(blocks[0]) - 1, block_coordinates=blocks[4]
        )
        mass_before = np.sum(
            blocks[2] * GAUSSIAN_VOLUME * np.sqrt(np.linalg.det(blocks[1]))
        )
        mass_after = np.sum(
            merged[2] * GAUSSIAN_VOLUME * np.sqrt(np.linalg.det(merged[1]))
        )
        assert len(merged[0]) == len(blocks[0]) - 1
...
def main() -> None:
    ...
    parser.add_argument("--grid-block-size", type=int, default=4)
    parser.add_argument("--grid-spatial-sigma-ratio", type=float, default=0.55)
    ...
    if args.self_check:
        ...
        return
    outputs = (args.output_npz, args.output_json, args.output_report)
    if any(path is None for path in outputs):
        parser.error("all output paths are required")
    if bool(args.ply) == bool(args.grid):
        parser.error("exactly one of --ply or --grid is required")
    if args.ply and args.prepare_report is None:
        parser.error("--prepare-report is required with --ply")
    if (
        args.target_count <= 0
        or args.macro_width <= 0
        or args.grid_block_size <= 0
        ...
        or not 0.0 <= args.organic_jitter <= 1.0
    ):
        parser.error(
