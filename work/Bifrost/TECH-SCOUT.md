...
|---|---|---|---|---|---|---|---|
| **B1** | **Physarum 黏菌自组织网络** | Jeff Jones 2010 + SebLague/Slime-Simulation + 一堆 WebGPU 10M-agent 实现 | ★★★ | ★★★ | ★★★ | ★★ | GPU compute 百万 agent 自组织成"活着的运输网络"，会生长、会找最短路。**所有人都在 shadertoy 里玩 2D，几乎没人把它做成一个 3D 场景的灵魂。** |
| **B2** | **铁磁流体 ferrofluid（磁场尖刺）** | SIGGRAPH 2019 Huang + 2024 IoB 磁静解算器 + UE5 原型 | ★★ | ★★★ | ★★★ | ★★★ | 磁场驱动的自组织尖刺黑液，攀爬、分裂、结晶。**实时 demo 里基本无人做**，独占到没边；代价是模拟偏重、R&D 最厚。 |
...
|---|---|---|---|---|---|---|---|
| **D1** | **Radiance Cascades（无噪实时 GI）** | 2024, Sannikov《流放之路2》；arXiv 2408.14425；jason.today/rc、tmpvar、radiance.wiki 一堆开源走查 + Godot 移植 | ★★★ | ★★★ | ★★★ | ★★ | **本轮头号推荐。** 够新（2024 才火）、能亲手造（走查/移植满地）、**还没进 UE 当复选框**。TA 自己撸出 Radiance Cascades 是真·硬 flex，画面还是无噪流光的高级感。 |
...
**Part D 的一句话判断：** 真前沿里真正踩中你甜点区的只有 **D1 Radiance Cascades（首推）+ D2 Flow Map 流体 + D3 风格化 GS** 这三张；D4 是精致窄口；D5/D6 够新但已滑向"底座/基建"，不建议当手艺亮点押。**如果只挑一张"既新又能亲手造还独占"的——就是 D1。**
...
| 牌 | 技术 | 一句话 | R&D |
|---|---|---|---|
| **D1** | Radiance Cascades 无噪实时 GI | 全表最优单张牌，2024 新、开源走查满地、还没变复选框 | ★★ |
...
> [!NOTE] 非拍板倾向（仅供参考）：
> - 单张"既新+能亲手造+独占+风险可控"的最优解仍是 **D1 Radiance Cascades**；**E1 GPU Work Graphs** 与之并列（2024 真·新硬件特性、艺术向几乎空白）。
