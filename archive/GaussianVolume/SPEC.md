...
> 状态：2026-07-31，当前冻结资产为 S10：原始标准 3DGS 15K geometry/opacity + G35 视觉链 + G37 runtime，`311,993 × 64 B`。
> 当前 Gate：G35 已通过用户视觉验收；G37 将 GS total 降至 `1.093 ms`；G38 测得单体积净新增 RHI working set 为 GS=`66.476 MiB`、SVT=`305.566 MiB`。结论只覆盖当前中远景静态云和 UE SVT U8。
> 当前执行顺序：主线已收尾；仅在需要 Shipping headline 时补 Shipping 样本，不恢复 H、G2 或 G28 路线，也不扩大到近景 Hero、动画、多光源或 NanoVDB 胜出声明。
> 历史 7DRGS Gate：H11 image-space `J` loss 判负；H12 改用逐点 light-space T 并修复 `Dual SH=false`；H13 六轴 tau 的数据、上传和 shader 消费已验证。两项当时未完成最终视觉签字，现只作历史证据。
...
3. 在同质量、同灯光下减少 volume pass 与完整帧 GPU 时间；
4. 满足 1080p 实时帧预算。

主张固定为 **matched-quality 下的总运行时消耗优势**：GPU memory 是已存在的强结构窗口，GPU 时间是必须用理论门槛＋RTX 5060 实测闭环的第二条正向证据。两者都不单独包装成算法创新点，也不得用一个维度的胜出掩盖另一个维度的失败。
...
理论门槛由 `mvp/simulate_gaussian_vs_vdb.py` 固定：50K/48 B＋512K pool 的完整 runtime representation=`4.320 MiB`，相对 NanoVDB Fp8=`2.22×`；2× 内存优势的最大预算=`60,330 kernels`。当前中远景工作量为 `24.85` 个理想解析命中/pixel，对 dense/SVT `140.87` samples/ray、NanoVDB `43.87–54.12` expensive samples/ray；相对 NanoVDB 的解析积分成本 break-even=`1.77–2.18×`。这些数字只建立可赢窗口，不能代替同质量 GPU 毫秒实测。
...
3. **Stage C — global contracted-kernel 50K fit**：先用全局 adaptive organic partition 按质量与空间多模态残差递归拆分到 exact `50K` leaves；强各向异性节点保持 PCA 主轴，近各向同性节点才随机扰动切面与质量二分相位，每个 leaf 做质量守恒 moment match。禁止固定宽度、轴对齐且彼此独立的 macroblock 输出，也禁止严格 50:50 kd-tree 重新引入规则节律。该 initializer 再以 progressive frequency supervision 优化 xyz、scale、rotation 与非负 `sigma_t`；Gabor 只用于离线 loss，最终仍导出 `omega=0`，不创建运行时 MLP、7D covariance、SH 或光照参数。
...
- [x] 用各向异性置信度控制的 organic partition 打散严格 PCA/50:50 节律，并以离线 Gabor-energy loss 训练 exact 50K；V3 相对旧训练 50K 的 τ PSNR/MAE、edge 与频率细节均改善，foreground-T PSNR 仍差 `0.666 dB`，等待 UE visual Gate 决定是否替换；
...
## 11. S3 G28 closeout status（历史节点，已被 G31～G38 取代）

S3 的当前 Gate 已关闭并通过，验收目标限定为中景/远景静态体积代理，而不是近景 Hero 或
SVT 的严格替代。通过条件是：大轮廓起伏可读、内部体积层次保留、外圈连续衰减、方向光与
天光重光照有效，且规则点阵不再成为主导瑕疵。

冻结基线是 `S3_ViewBoundaryMorph_Mask125_150_MassNormalized_FJ.ply`（SHA256
`5F0F3F2D4D72523026382966073B04CAE464780DF1DF21EF1E9C9483AF4421B`，`311,993` 点，
`64 B/point`，约 `19.043 MiB`）。它由 VDB teacher、标准 3DGS 15K 几何、12.5% /
150-step view-boundary 轮廓候选和 `J^0.4` compact transport 构成，UE 侧走
preprocess → sort → HW Quad → shared-opacity composite → DGSM/phase。

冻结参数为 opacity=`0.4`、power=`1.0`、relight=`0.3`、ambient=`0.01`、DGSM
density=`0.45`，phase、Actor transform 和点数不变。AlphaCutoff、内部 alpha、统一
footprint 放大和全图 blur 仅作为失败实验保留。该段记录 G28 当时状态；当前冻结资产和结论
以下方 G35/G37/G38 为准。

## 12. G35/G37/G38 最终收尾状态（2026-07-31）

- 当前资产是标准 3DGS 15K 原始 geometry/opacity + VDB direct `J^0.4` 六轴 transport，
  `311,993 × 64 B`，SHA256=
  `AE7177BF3753E9905C34208A9D46A2647018F55A49FF13581A717BA1040EA0FB`；G24～G28
  boundary-morph 已回滚为历史分支。
- G35 的结构 coverage 桥接、轮廓切线和内部 coverage/depth joint bilateral 已通过用户
  自由镜头视觉验收；G34 梳状方向模糊、强 AlphaCutoff 和全图 blur 均保持否决。
- G37 alpha-support quad crop 不改变保留像素和视觉链。固定同帧中位数：SVT feature=
  `3.241 ms`、GS=`1.093 ms`；相对 G35，GS total 降 `18.6%`、HW Raster 降 `30.8%`。
- G38 的 3×3 独立冷进程测得单体积净新增 RHI working set：SVT=`305.566 MiB`、
  GS=`66.476 MiB`；GS 节省 `239.090 MiB / 78.245%`，约为 SVT 的 `1/4.597`。
  GS 分解为 non-transient=`19.063 MiB`、transient 净增=`47.414 MiB`。
- 整进程专用显存中位数 SVT=`2664.178 MiB`、GS=`2343.980 MiB`，节省
  `320.198 MiB / 12.019%`；这里的 2.3 GiB 级数字包含整个 UE 场景，禁止描述成单个 GS。
- 当前正结论限定为 Development、当前中远景静态云、当前 UE SVT U8、同机位同灯光。
  NanoVDB、近景 Hero、动画、多光源与 Shipping headline 仍不在已证明范围。
- 事实源：`SOP-VDB-TO-GS-G35.md`、`PERFORMANCE-VDB-VS-G35-G37-20260731.md`、
  `VRAM-COLD-G37-VS-SVT-20260731.md` 和
