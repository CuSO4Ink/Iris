...
### 2026-07-09 — [决策] D3-KERNEL §2.5 层 2 触发判据定为路线 C
恒星层 2（marching cubes 局部 mesh）"该在哪触发"三路线拍板：路线 C（screen-space 轮廓粗筛 + world-space 噪声场二次筛选）。否 A（纯屏幕轮廓）因判据与层 1 材质纹路弱耦合，易出现"轮廓 mesh 位置与翻涌纹路对不上"的贴皮感，削弱前沿内核 breakdown 叙事；否 B（纯世界空间场）因额外 compute pass 开销未知，对已绷紧的 5 周工时是隐患。C 用 A 的低开销粗筛限制 B 的开销范围，兼顾性能可控与"磁场驱动真实翻涌"叙事强度。

### 2026-07-09 — [发现] `NS_Slime` 密度场输入 = 外部 3D SDF，非任意噪声值
用户在 UE 里查证 `NS_Slime` 节点图，确认其 marching cubes 密度场输入接受**外部 3D SDF（有符号距离场）**，比预期的"任意 volume texture/噪声源"更规整。影响：路线 C 里层 1 材质的域扭曲噪声（标量场）不能直接接入，需先补一层"噪声阈值化→SDF"转换才能喂给 `NS_Slime`，这一层转换成本要计入 W2 管线设计和 §4 性能预算。
...
### 2026-07-09 14:45 — [决策] 尺度校准巨星移至 +Y 海侧，"海天恒星"构图确认保留
`P0_ScaleTest_L` 巨星 `ST_Star_GIANT_D3000m` 原置于 Y=-1800m（悬崖侧，纯尺度校准无方向意义），本人在编辑器内直接查看 +Y 海侧构图后确认保留该朝向。该巨星本身仍是校准道具（非正式走廊内容），但其位置作为"恒星应在海面地平线方向"的构图参照对后续 T-L3.1 等离子恒星本体的最终地平线方位角有指导意义——**建议 T-L3.1 落地时优先在 +Y（海面）方向定位**，覆盖此前 §6 待拍板项。位置改动需在编辑器手动 Ctrl+S 保存（当时引擎正在重新编译，MCP 网关不可用，无法二次核验保存状态，需用户自行确认）。

### 2026-07-09 14:45 — [决策] 背景山体/岩石摆放验收通过
`SM_Background_A/B`（远景山体）与 `SM_Rock_A/B`（岩石）经修复包围盒重叠、消除与尺度校准巨星的物理穿插后，本人验收通过，作为走廊远景/悬崖点缀定案保留。
用户用自备角色蓝图 + GameMode（非默认 ThirdPersonCharacter）接入 `L_Bifrost`，在 PIE 里从 S1 出生点实跑到 S4 尾端，确认不掉海、不穿墙、不卡 Gate、帧率不塌，本人口头验收通过。据此回填归档前任务记录 T-P0.2 完成，及 `T-P0.1`"玩家路径可通行"验收项（HANDBOOK §2 表格 + blockout-v2-measured.md §3 checklist）。W1 三件套（T-P0.0 路径设计 / T-P0.1 blockout / T-P0.2 第三人称通行）全部闭环，可进入 W1 收尾（MCP 环境验证/最小母材质 demo）或直接为 W2 量产做准备。
...
落地过程中 AI 先按 `PIPELINE.md` 及归档前任务记录里"3 张母材质（沙/岩石/遗迹金属）"的旧措辞建了 `M_Rock`，与 07-07 21:10 条目本来定的"环境 uber 材质"架构方向不一致——用户发现后拍板纠正："一整个全局环境的材质，名字就叫 `M_Env_Uber`，所有 feature 都写进去，要什么效果用材质实例调"。据此撤销 3 张母材质分工，收口为单一 uber 架构：
...
- 已删除被取代的 `M_Rock`。同步回填 `PIPELINE.md`/`ROADMAP.md`/`AI-BRIEF.md`/`HANDBOOK.md` 等相关文档，将"3 张母材质"收口为 uber 架构描述。当前色值/参数取值为占位估算，节点图与 LookDev 微调仍需用户验收（HANDBOOK §3 已标验收动作）。
...
### 2026-07-10 — [发现] Abyss UE AgentSkill 已接入 Bifrost 项目规则
按 Iris L2 文档与 HANDBOOK/ROADMAP 压缩出 UE 侧项目技能 `/Game/Skills/BifrostProjectSkill.BifrostProjectSkill_C`，覆盖 `/Game/Bifrost` 命名空间、`L_Bifrost` 默认关卡、4 段坐标、W2/W3 当前重点、MCP 批量铁律、transform 单位坑、审美边界与已知资产。后续 Abyss UE Agent 处理 Bifrost 资产/Actor/材质/灯光/自动化时应先应用该技能。

### 2026-07-10 — [决策] 技术图收敛为 Stellar Gaussian Field
项目从第三人称海岸关卡收敛为固定镜头优先的实时场景作品。核心链路为：恒星状态 -> spline/宏观参数 -> Gaussian primitive -> GaussianVolume 解析体积渲染 -> 场景画面；轻量 ONNX/NNE decoder 只在 renderer 与 field 创作接口验证后，用于 primitive 局部动态残差。第三人称、S1-S4 跑图、铁磁流体、通用 Raymarch、HPVolumeCloud 移植、Marching Cubes Layer 2、VDB proxy、场景扫描 3DGS、Landscape/PCG 全部移出当前排期，保留为历史研究或资产，不删除。

### 2026-07-10 — [修正] Raymarch / HPVolumeCloud / Marching Cubes 是延后模块，不是废弃项
三者不进入 Gaussian Field 的基础 G0-G3 验收，以免重新分散核心实现；但它们都可在 G2 后按最终画面缺口恢复。自定义 Raymarch 用于局部云/极光介质，HPVolumeCloud 用于提炼并 UE 适配云体积思路，Marching Cubes 用于恒星轮廓/日珥局部强化。恢复时一次只选一个，并先验证性能和与恒星因果链的关系。

### 2026-07-10 — [决策] 当前转为 TechLab 逐项验收，不推进场景
场景构图、资产摆放和最终镜头暂时全部暂停。所有保留技术先在 `L_Bifrost_TechLab` 达到正确性、可控性、性能和证据四项标准；Raymarch、HPVolumeCloud、Marching Cubes 不再等待场景缺口，而是作为独立技术模块完成验收。场景集成只在技术模块全部评估后重新开启。
### 2026-07-12 — [决策] 恢复 S3 固定镜头构图并完成第一轮重排

用户明确恢复 Bifrost 场景摆放工作，范围限定为利用 actor transform、world bounds、层级和固定镜头重排现有资产，不要求输出空间认知过程。实读 `L_Bifrost` 后确认主要问题是 7 段 cliff 同 Y/同高度等距排队，blockout gate/边界墙与 ScaleTest 参照仍可见，导致画面像规则灰模走廊。

已保留四段 Floor，隐藏其余 BLK 边界/gate/anchor/marker 与 ScaleTest 参照（只设组件不可见，不删除）；将 S3 cliff 分配到左右前中后景，column 收为近景剪影与中景节奏点，rock 移出中心视线；GaussianVolume Actor 从场景外移回 S3 轮廓线上方；巨星缩小并偏置为远景焦点。Hero viewport 固定在 `(28000,-6000,700)`、`Pitch=13/Yaw=87.7`，关卡已保存。仍不恢复第三人称、S1-S4 跑图或全场景扩张。
### 2026-07-12 — [回滚+实现] 恢复巨物压迫尺度并接入 S3 岸边浪最小组合

用户指出第一轮构图把 `Bifrost_S3_Lordenfel_Cliff` 从 7 倍缩到 3.5 倍，削弱巨大悬崖；恒星按普通远景处理也偏离“近距离天体压迫感”。已恢复 Lordenfel Cliff 为 7.5 倍并移到侧向框景；恒星改为 2400 倍、近距离覆盖大部分天空，保留清晰边缘而非普通远景圆盘。

海水审计确认 `M_Wave_Base_Inst` 已暴露 `CoastTex`、`CoastOffset`、`WaveForwardIntensity`、`SDFThreshold`、`LandScapePosition&Size`、HeightTex 与泡沫/深度参数；`BP_OceanInitial` 是空壳，真实组合为高细分水面/`NS_InfiniteMesh` + `BP_HeightMapCreate` 正交捕获。修复 Bifrost 材质实例 Coast/Wave/Height 纹理与 HeightMap BP 的 RT/MPC 浅拷贝旧路径；在 `L_Bifrost` 放置 400m×200m `SM_FluxPlane1024x1024` Hero 海面和 400m Height Capture，海面/岸线基准 Z=-40cm，材质 Debug 从 20 归零。PIE 已通过，水面 WPO/时间动画链运行；Niagara 九级 clipmap 暂未接入，Hero 固定镜头由单张高细分网格覆盖。
### 2026-07-13 — [决策] 暂停临场式 AI 摆景，先建设场景编辑支撑结构

用户明确评价当前 AI 场景编辑能力与最终画面仍不够好，计划在新上下文构建一套更严密、有证据支撑的结构，再让 AI 接入场景工作。该判断覆盖此前“S3 第一轮重排/岸边浪最小接入”的完成表述：技术链与资产变更可保留，但**当前构图和 LookDev 不视为审美验收通过**。
