...
> 当前主线对象：`/Game/SSPR_Validation/M3/AnisotropicSplat_V4_Dev/NS_SSPR_AnisotropicSplat_V4_Dev`
>
> 数值来自 2026-07-30 对 Dense G5 恢复基线的有效值读回；M3 主线由 V4 Dense 冻结源复制并继承这组参数。`/Game/SSPR_Validation/Performance/DenseG5SparseV2/NS_SSPR_AnisotropicSplat_Main` 仅是使用同参数的旁路性能候选。
...
当前 M3 主线仍使用以上数值与 Dense Raster，最大候选数为
`49×11=539/粒子`。Sparse V2 旁路候选使用质量守恒的
`33×7=231/粒子`，最大原子写入候选下降 `57.14%`，但尚无同机位性能/视觉 Gate，不能据此宣称主线已优化。
