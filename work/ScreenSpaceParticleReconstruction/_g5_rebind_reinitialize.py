...
# Rebinding the same asset through None repeatedly leaves obsolete component
# DI override subobjects serialized in the validation level. For an unchanged
# System asset, keep the current component binding and reinitialize in place.
# A real DI interface/schema change must use a one-time clean actor replacement
# rather than accumulating another generation of overrides here.
component.deactivate()
current_asset = component.get_asset()
if current_asset is None or current_asset.get_path_name() != SYSTEM:
...
if not raster_interfaces:
    raise RuntimeError("No live RasterizationGrid3D clone found")
if len(render_target_interfaces) < 2:
    raise RuntimeError(
        "Expected current-frame Main and Aux RenderTarget2D clones"
    )
...
try:
    level_saved = bool(unreal.EditorLevelLibrary.save_current_level())
except Exception:
    level_saved = False
...
print("G5_REBIND_REINITIALIZE=" + json.dumps(result, sort_keys=True))
if not result["active"] or not system_saved:
    raise RuntimeError("G5 rebind/reinitialize gate failed: " + repr(result))
