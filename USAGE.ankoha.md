...
- 需要留下长期信息时，直接让 AI 更新项目 LOG、BACKLOG 或 AI-BRIEF
...
AI 会提炼当天 LOG 的关键决策更新到 AI-BRIEF，清理已完成的普通任务并保留项目约定的状态/验收记录，给你一份摘要确认。
...
```
/project-init 项目代号 --ue
```

除了三件套，还会在 `AI-BRIEF.md` 顶部写入 UE 标记和 UEAgent-first 前置导航；后续接入时先走 UEAgent 路由。
...
| **项目生产**  |                          |                                                 |
| 新建项目      | `/project-init X [--ue]` | 三件套建在 `work/X/`；`--ue` 额外写入 UE 标记和 UEAgent 前置导航 |
| 收工快照      | `/checkpoint`            | LOG 提炼到 BRIEF + 清理普通完成项 + 摘要                    |
| **维护/诊断** |                          |                                                 |
| 忘了命令      | `/help`                  | 完整指令表复述                                         |
