"""
VDB 鈫?Gaussian 杞崲鍣ㄦ祴璇?
鐢ㄧ▼搴忓寲鐢熸垚鐨?density volume 妯℃嫙 VDB grid锛屾祴璇曡浆鎹㈠櫒鐨勶細
  1. 鍩烘湰杞崲锛堝悇鍚戝悓鎬э級
  2. 鍚勫悜寮傛€фā寮忥紙gradient-based锛?  3. max_primitives 闄愬埗
  4. 娓叉煋楠岃瘉锛堣浆鎹㈠悗鐨?GaussianCloud 娓叉煋鏁堟灉锛?  5. 缁熻杈撳嚭
"""
import sys, os, time
import numpy as np
import matplotlib
...
def make_synthetic_vdb(shape=(24, 24, 24), voxel_size=0.2):
    """鐢熸垚涓€涓▼搴忓寲鐨勪綋绉簯 density volume銆?
    浣跨敤澶氫釜鐞冪姸瀵嗗害鍥㈠彔鍔狅紝妯℃嫙鑷劧浜戝舰鎬併€?    """
...
def main():
    ...
    # 1. 鐢熸垚 synthetic VDB-like density volume
    print("=" * 60)
    print("VDB 鈫?Gaussian Converter Test")
    print("=" * 60)
    dense, voxel_size, origin = make_synthetic_vdb(shape=(24, 24, 24), voxel_size=0.2)
    ...
    print(f"  compression: {stats_iso['compression_ratio']:.1f}x "
          f"({stats_iso['active_voxels']} voxels 鈫?{stats_iso['n_primitives']} Gaussians)")
    ...
    # 4. max_primitives 闄愬埗
    print("\n--- Test 3: max_primitives=512 ---")
    cloud_limited = vdb_to_gaussians(dense, voxel_size, origin, density_threshold=0.3, density_scale=2.0, sigma_max_scale=2.0, max_primitives=512)
    cloud_limited_aniso = vdb_to_gaussians(dense, voxel_size, origin, density_threshold=0.3, density_scale=2.0, sigma_max_scale=2.0, anisotropy_mode='gradient', anisotropy_ratio=0.3, max_primitives=512)
    ...
    # 娓叉煋鍚勫悜鍚屾€х増
    t0 = time.time()
    tau_light = precompute_light_tau_matrix(cloud_limited, light_dir)
    res_iso = render(cloud_limited, cam, target, 45.0, W, H,
                     light_dir, light_color, ambient, tau_light=tau_light)
    dt_iso = time.time() - t0
    print(f"  [isotropic] N={cloud_limited.N} {W}x{H} took {dt_iso:.2f}s ({W*H/dt_iso:.0f} ray/s)")

    # 娓叉煋鍚勫悜寮傛€х増
    t0 = time.time()
    tau_light_a = precompute_light_tau_matrix(cloud_limited_aniso, light_dir)
    res_aniso = render(cloud_limited_aniso, cam, target, 45.0, W, H,
                       light_dir, light_color, ambient, tau_light=tau_light_a)
    dt_aniso = time.time() - t0
    print(f"  [anisotropic] N={cloud_limited_aniso.N} {W}x{H} took {dt_aniso:.2f}s ({W*H/dt_aniso:.0f} ray/s)")
    ...
    axes[0, 0].set_title(f"Isotropic (N={cloud_limited.N})")
    axes[0, 0].axis("off")

    axes[0, 1].imshow(to_srgb(res_aniso["color"]))
    axes[0, 1].set_title(f"Anisotropic (N={cloud_limited_aniso.N})")
    axes[0, 1].axis("off")
    ...
    plt.suptitle("VDB 鈫?Gaussian Converter: Render Validation", fontsize=14)
    plt.tight_layout()
    ...
    # Gaussian centers near z=mid slice
    ...
    axes[1].set_title("Gaussian centers (z鈮坢id slice)")
    axes[1].set_aspect("equal")

    # 瀵嗗害鐩存柟鍥?    axes[2].hist(cloud_limited.sigma_t, bins=50, color="steelblue", alpha=0.7)
