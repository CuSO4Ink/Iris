...
H13 切到 `LightTransmittance` debug view 后出现稳定、明显的黑白方向梯度；切回 `Final` 也有方向明暗。该证据把故障边界收窄为视觉强度调校：六轴 tau 资产、GPU 上传与 shader 消费链路均已接通，不再重复训练或重接 transport。最后可靠读回为 Density=`1.32799995`、Gamma=`1.245066`、Directional Shadow Density Scale=`0.304266`；最终强度仍由用户签字。
...
最后可靠现场状态为 H12、H13、SVT 均 visible，H0 不在当前关卡。UE 地图、导入 PLY/JSON 和 Actor 实例覆盖不属于 Iris Git；关卡是否已保存不能由仓库推断。为消除代码复现缺口，已把本地 clean-room `GaussianSplattingForUnrealEngine 0.1-reconstruction` 的 21 个源码／shader 文件（不含 Content 资产）快照到 `ue-plugin/GaussianSplattingForUnrealEngine/`。后续未创建 H14，也未继续该训练线。

### 2026-07-28 — [WDAS 404K][已部署][待用户视觉 Gate] 固定预算去蓝／对比度／细节候选

旧 404K 宽核候选只是 quarter-grid block moments，没有完成用户要求的固定预算画质恢复。重新比较同一 held-out 协议后，half-grid `8³`、spatial sigma=`0.4` 的 exact `404,524` 候选为当前最优：foreground-T PSNR=`35.62960 dB`、τ PSNR=`42.17195 dB`、τ MAE=`0.07028`、edge L1=`0.058744`、IoU=`0.98655`。相对旧宽核基线分别改善约 `+3.60 dB`、`+3.55 dB` 和 `-28.8%` edge error。40-step extinction-only probe 虽使 τ PSNR `42.0326→42.0451 dB`，但 foreground-T 和 edge 同时回退，numeric Gate=`false`，已否决。

新增 initializer→compact PLY 的最短导出路径，复用既有 64 B layout，并从源 grid 烘焙六轴 light τ；输出 `WDAS_Cloud_Half_B8_Balanced_CompactTransportGS_404K.ply` 为 exact `404,524` records、payload=`25,889,536 B`、SHA256=`482b53c72728efc2fb78528dc951000bb8536ee4905500198f59090aba4f2e39`。10 项相关单元测试和 Python compile 通过。

发蓝／发虚的共享根因位于 Lumen indirect 合成：旧路径硬编码 intensity=`1`，绕过组件已有 `AmbientLightIntensityScale=0.025` 和 tint。现将两者作为常量 uniform 传入 latent／legacy composite，不增加 per-point、DGSM 或 texture 数据。`AbyssEditor Win64 Development` 构建成功，D3D12 冷编译 `FGSCompositePS` 成功。TechLab 已保存 Actor `Compact Transport GS + DGSM | WDAS Balanced / 404K`；地图检查 0 error/0 warning，offscreen runtime 无 shader/RDG/GPU crash/fatal。最终颜色、对比度、细节和重光照只等待用户 live viewport 签字；签字前不增加到 600–800K，也不发布同质量性能 headline。

用户随后确认该版“效果好很多”，并要求保留为后续优化失败时的保底。已冻结 Gate `WDAS404K-BALANCED-G1` 到 `artifacts/gates/2026-07-28_wdas_404k_balanced/`：包含 exact PLY、已保存 TechLab map、生成 initializer/report，以及 Lumen indirect 修复涉及的五个插件源码／shader 文件；全部记录 SHA256。后续候选只允许在新文件中推进，不原位覆盖该 Gate。

固定预算后续扫描只改变核宽和体积守恒的各向异性，不增加记录或字段。G2 `sigma=0.38 + anisotropy boost=1.15` 在与 G1 相同的 1,000 held-out rays 上同时改善：foreground-T `35.1396→35.6227 dB`、τ PSNR `42.6639→42.6659 dB`、τ MAE `0.06117→0.06018`、edge `0.06115→0.05944`、Gabor energy/phase `0.001784/0.003319→0.001511/0.002945`。导出仍为 `404,524` records、`64 B/record`、payload=`25,889,536 B`；中心和六向 transport 相对 G1 max error=`0`，SHA256=`C4321FCEC040103DF4A615E0598D488F31541F22F2E77007289B4B9EFA564FDF`。当前 UE 编辑器仍打开，因此没有覆盖关卡，等待用户关闭后再部署视觉候选。

用户关闭 UE 后已部署 G2。`G2CompactTransportDeploy.log` 记录新 PLY 加载、map check=`0 error / 0 warning`、Actor=`Compact Transport GS + DGSM | WDAS G2 Sigma038 Aniso115 / 404K`、records=`404,524`、saved=`true`。第二个独立 D3D12 进程重开 TechLab 后再次加载同一 PLY 并 readback 新标签；无 shader/RDG/GPU crash/fatal。部署只替换 PLY 和标签，用户当前视觉参数保持 `opacity multiplier=0.6`、power=`0.9`、relight scale=`0.7`、ambient scale=`0.01`，最终晋升仍等待 live viewport。

用户在 G2 live viewport 截图中确认规则点阵仍明显。P0 的普通 geometry 训练只让中心中位移动 `0.04 cm`，不足以破坏约 `6.5 cm` block pitch，且 held-out Gate 失败。J15 随后用 block 内 `15%` deterministic stratified jitter 将中心中位移动提高到 `0.96 cm`，60-step 后虽相对自身初始值全指标微升，但相对 G2 的 τ/T/edge/Gabor 全面回退；`25/50/75%` update line search 也全部失败，未导出、未部署。

首次 J15 前台训练因用户中断对话在 step 20/60 被终止。用户新增执行规则：任何预计超过 5 分钟的训练必须事先同步内容与耗时；长任务用独立隐藏后台进程和日志，使对话中断不再终止训练。J15 rerun 已按该方式完成。下一候选是从 25% 轻抖动 initializer 直接恢复训练，预计 `6–8 min`，等待用户批准后启动。

用户准备休息并授权不依赖视觉反馈的夜间数值搜索。新增可恢复执行器 `mvp/run_degrid_overnight.py`，self-check 与 Python compile 通过；固定运行 `1/2/3/4%` block jitter × 两档保守 recovery，共 8 个 exact 404K 候选，预计 `55–75 min`。每个候选原子落盘 summary，完整候选自动跳过；strict Gate 要求相对 G2 七项 held-out 指标全部不退，bounded Gate 只保留次日视觉候选且禁止自动部署。后台 PID=`89780`；heartbeat automation `gaussianvolume` 每 15 分钟监控、异常续跑、terminal 后自停。

### 2026-07-29 — [G4][训练完成][否决] 404K 夜间 de-grid 搜索

夜间搜索用时 `126.26 min` 完成 8/8 候选，stderr 为空；全部保持 exact `404,524`、finite、positive-definite covariance 和质量守恒。conservative/recovery 两档的 `1/2/3/4%` jitter 均未通过 strict 或 bounded Gate。最接近的 conservative 1% 相对 G2 改善 τ PSNR/MAE、foreground-T 和 Gabor，但 edge=`0.059463` 略差于基线 `0.059441`，且 lattice-order 只从 `0.810900` 降至 `0.810369`（`0.066%`，远低于 bounded 要求的 `2%`）。最大 4% jitter 也只降低约 `1.049%` lattice-order，同时 τ/T/edge/Gabor 明显回退；recovery 档未改变这一趋势。

`selected_strict_candidate=null`，strict/bounded 集合均为空。按自动 Gate 停止：未生成新 compact PLY、未重烘 transport、未修改或部署 UE；G1 Gate 备份与当前 G2 保持不变。完整逐候选指标和失败原因已写入 `IMPLEMENTATION-AND-OPTIMIZATION-LEDGER.md`，机器结果位于 `artifacts/wdas_404k_degrid_overnight/summary.json`。

### 2026-07-29 — [Density De-grid P0][通过] 404K 三维空间索引

用户批准修订 SPEC 的 P0 后，新增最小 `3σ AABB` regular-bin evaluator 和回归检查；同 support cutoff 下 indexed/brute 最大误差=`0`，空 bin、边界、finite gradient 与超大 support guard 全部通过。404K 上扫描 `10/13/16 cm` bin，10 cm 的 field 时间与峰值显存最优，因此冻结为 P1 默认。

RTX 5060、16,384 个连续三维 sample 的正式 benchmark：50K build/forward/backward=`93.85/2.22/2.06 ms`、peak allocated=`42.05 MiB`；404,524 build/forward/backward=`829.86/7.62/15.11 ms`、peak allocated=`184.34 MiB`，无 OOM，梯度全部 finite。机器证据位于 `artifacts/wdas_density_degrid/p0_spatial_index/`。P0 通过只证明底层 evaluator 可行；未启动 P1/P2、未修改或部署 UE，空间索引 rebuild interval 等 P1 首个优化步的最大中心位移实测后再定。

### 2026-07-29 — [Density De-grid P1][通过] 自由几何训练器核心

按批准范围在同一个 trainer 文件内完成 absolute center/log-scale/quaternion/softplus extinction、连续 active/edge/empty source sampling、三维 density/coverage/empty/mass loss、解析 ray τ 与既有 T/edge/Gabor loss 接口、高残差 densify/低质量 prune，以及包含 Adam 和 Python/NumPy/Torch CPU/CUDA RNG 的原子 checkpoint。合成 `continuous step 2` 与 `step 1 save/load → step 2` 的 sample 流完全一致、参数 max error=`0`；局部 density Hessian covariance 的验证 anisotropy ratio=`4.916`，预算 `2→3` 后 covariance/extinction 均为正。

真实 WDAS 50K/4096-sample 单步用时=`0.346 s`、GPU peak allocated=`46.49 MiB`，所有参数 finite，minimum covariance eigenvalue=`1.17e-5`；geometry LR=`5e-4` 时最大中心位移=`0.0866 cm`，所以 P2 初版每步 rebuild spatial index。末尾 GPU batched eigvalsh 曾触发 cuSOLVER 参数错误，训练步没有失败；离线结构检查切换为导出链同口径的 NumPy CPU 后通过。证据位于 `artifacts/wdas_density_degrid/p1_trainer_selfcheck/`。尚未启动 P2、生成 compact PLY 或修改 UE。

### 2026-07-29 — [Density De-grid P2][准备通过][等待正式训练] 50K ROI 合成链

按用户批准完成 P2 最小调度器与固定输入：exact 50K 连通 ROI、354,524
个冻结 G2 点、50,000-cell core、122,928-cell halo、64 个 ROI held-out
patches，以及独立 40 个全局 patches（1,000 rays）；全部数组和源输入均冻结
SHA256。调度固定为 `10K→20K→30K→40K→50K`，每轮执行三维 field fit、
ray recovery、global/ROI evaluation 与误差 densification；checkpoint 保存
round/phase/step、Adam、完整 RNG 和 hashes，可从当前 phase 恢复。

512 点真实链 dry-run 通过：split/full field 最大误差=`2.384e-7`，
中心最大位移=`0.0866 cm`，ROI 外回退点=`0`，hybrid ray finite/非负，
GPU peak allocated=`143.72 MiB`，耗时=`5.35 s`。代码 compile、trainer
resume 回归和 CLI 检查通过。正式默认每轮 `80 field + 5 ray` steps，
保守 ETA=`25–45 min`；因 UE 当前允许用户继续使用，尚未启动正式 GPU
训练，也未生成 PLY、重烘 transport 或修改 UE。

### 2026-07-29 — [Density De-grid P2][训练完成][否决] 自由 50K ROI

用户关闭 UE 后启动 `10K→20K→30K→40K→50K` 正式 P2。round 0 的
80 field + 5 ray steps 和 evaluation 完成后，候选 NPZ 保存暴露
trainable center 未 `detach()` 的边界错误；checkpoint 完整停在
`round 0 / evaluation`。修复公共保存函数及 densification/最终输出的
同类调用、补充导出回归并通过后，以 `--resume` 从原 phase 继续，没有重跑
round 0 优化。恢复进程用时 `3.07 min`，总有效训练/评估约 `4 min`；
stderr 为空。

最终 lattice-order 从 G2 ROI 的 `0.793240` 降到 `0.016421`，说明自由
几何确实消除了 B8 相位；但 screen-space 质量 Gate 失败。全局 τ
PSNR/MAE=`38.2649 dB / 0.089898`，相对基线
`42.3511 dB / 0.062894` 明显回退，edge=`0.063052` 也差于
`0.061049`。ROI τ PSNR/MAE=`35.0081 dB / 0.175599`，相对基线
`42.3031 dB / 0.073779` 回退，ROI edge=`0.039078`，基线为
`0.030191`。global strict/bounded 与 ROI Gate 全部为 false。

30K 是 τ 最佳中间 round；40K/50K densification 后 τ 反而回退，而 edge
仍小幅改善，说明当前每轮 `80 field + 5 ray` 的恢复配比不足，且新增点后的
全局质量归一需要加强。最终诊断 NPZ 保持 exact `404,524`、finite、
positive extinction 与 positive-definite covariance，SHA256=
`7461de2437c7afb2c27e7d929214a99c1332a5c34d2e9da7fb83331db10f045a`。
按 SPEC 停止：不进入 P3、不导出 compact PLY、不重烘 transport、不修改
或部署 UE；G1/G2 保持不变。

### 2026-07-29 — [Density De-grid P2b][准备通过][等待训练] 30K 小步恢复

P2b 固定复用 τ 最佳的 30K 中间候选，SHA256=
`057a4b87e391c3175b3c01a0be4efe06697d16390c475642171fc30bef2c0aaa`。
现有 trainer 增加可选 warm-start/input-root/schedule，而未复制第二套训练
框架；调度改为 30K recovery 后 `35K→40K→45K→50K`，每轮
`80 field + 40 ray`。每轮单独比较前一接受轮的 global/ROI τ MAE 与
edge；global 和 ROI τ 同时回退超过 `0.5%` 即停止，且 joint-regression
轮不能覆盖当前最佳候选。

checkpoint 继续保存 phase/round/step、Adam、RNG 和输入 hashes，并新增
warm-start/parent-metrics hashes。Python compile、trainer/resume/export
回归、joint τ Gate 与 30K CPU 装载→NPZ 再导出均通过；roundtrip exact
30K、字段 finite。按 P2 实测外推正式 ETA=`8–15 min`；当前只完成准备，
未启动 GPU 长训练、未修改 UE。

### 2026-07-29 — [Density De-grid P2b][训练完成][自动停止] best=35K

P2b 在 UE 关闭后运行 `2.7115 min`，stderr 为空。30K recovery 将
global/ROI τ MAE 从父候选的 `0.082189/0.160897` 改善到
`0.077857/0.135794`；35K 继续改善到 `0.076417/0.128172`，成为 best。
40K 则回退到 `0.079930/0.136811`，相对 35K 分别变差约
`4.60%/6.74%`，触发 joint τ regression 自动停止，因此没有运行
45K/50K。

best 35K 的 global τ PSNR/MAE=`40.0662 dB/0.076417`，ROI=
`37.5701 dB/0.128172`；相对 G2 baseline 的
`42.3511/0.062894` 与 `42.3031/0.073779` 仍明显失败。结构为
35,000 trainable + 354,524 frozen=`389,524`，全部 finite、positive
extinction、positive-definite covariance，lattice-order=`0.010843`，
SHA256=`63b467b698c5449ffa503706989de3b07aa2943ed91128890f2bd9d0df80bbe4`。
结论是加长 ray recovery 与 5K 小步增点确有正收益，但仍不足以追平 G2；
按 Gate 不进入 P3、不导出 compact、不烘 transport、不修改或部署 UE。

### 2026-07-29 — [Density De-grid P2c][训练完成][否决] 40K ray convergence

为排除 P2b 40K 只是暂未收敛，从其 exact 40K checkpoint 固定点数、关闭
field fit 与 densification，只追加 ray recovery；每 40 steps 独立保存和
held-out 评估，累计跑满 `200` steps。运行 `1.8521 min`，stderr 为空，
20/40-step checkpoint 与阶段恢复链正常。

global/ROI τ MAE 在 40/80/120/160/200 steps 分别为
`0.082205/0.165510`、`0.082116/0.161475`、
`0.081607/0.160232`、`0.081991/0.158974`、
`0.082347/0.160726`。这些结果均差于追加训练前 40K 的
`0.079930/0.136811`，更差于 35K best 的 `0.076417/0.128172`。
因此 40K 回退不是简单的训练步数不足：ray-only training patch loss 可继续
下降，但 held-out 明显过拟合/目标失配。best 保持 35K，不生成 40K/50K
最终资产，不修改或部署 UE；P3 继续禁止。

### 2026-07-29 — [Density De-grid P2b][诊断导出][非 Gate] 35K UE 样本

按用户要求，将 P2b best 35K 与冻结的 G2 ROI 外 `354,524` 点合成独立
`389,524` 点诊断资产，并从源 half-grid 重烘六轴 transport。导出仍是
compact static transport `64 B/record`，payload=`24,929,536 B`，
PLY SHA256=`586916c4c97eeafc88ede85c165e8e762244529fdffa9327b3a75ed863729aa7`。

结构校验通过：16 float 字段、全部 finite、positive opacity/extinction、
positive-definite covariance、transport 范围 `[0.06,1]`，PLY 中心和
covariance 相对 hybrid NPZ max error=`0`。独立副本已放入 Abyss 插件
`Content/Data/WDAS_FreeDensity35K_Hybrid389524_DIAGNOSTIC.ply`。随后按用户
要求通过 UEAgent MCP 在 TechLab 内存态新建同 transform 诊断 Actor，完整
复制 G2 的 Directional/Sky Light、DGSM、Dual SH、phase 与 opacity 参数，
日志确认加载 `389,524 compact static transport gaussians`。诊断 Actor 带
`UEAgentBatch_FreeDensity35K_20260729` 及语义 tags，位于
`GaussianVolume/Diagnostics`；原 G2 只设 `bVisible=false`。关卡未保存，
未覆盖 G1/G2；该样本只用于视觉 A/B，不改变 P2b numeric Gate 失败的裁决。

首次从 class 创建的诊断 Actor 虽然属性和 `389,524` 加载日志均通过，用户
仍报告视口空白并将其删除；删除后原 G2 仍为 `bVisible=false`，造成场景继续
为空。R2 先恢复 G2，再通过 `EditorActorSubsystem.duplicate_actor` 复制现有
G2 实例，只替换 PLY，避免遗漏 class 新建路径中的非公开初始化状态；当前
G2 参数（包括 `opacity=1 / power=1.2`）逐项保留。R2 结构读回为 G2
`bVisible=false`、诊断组件 `bVisible=true`，新 Actor 已在 Outliner 选中，
日志再次确认加载 `389,524`；关卡仍未保存。

### 2026-07-29 — [回滚] Density De-grid SPEC 失败并退役

UE 原位替换确认 P2b 35K 诊断云既偏离 VDB 造型又未解决可见点阵；结合
global/ROI numeric Gate 全部失败，正式将该 SPEC 标为失败并退役。其空间索引、
断点恢复和 ROI 评估基础设施可复用，但 P3 禁止启动，不能靠追加步数重开。

同时确认当前 renderer 只维护一个 active component，旧的双 Actor
visible/hidden 读回不能构成有效 A/B 证据；诊断关卡未保存，重开恢复原始 G2。

### 2026-07-29 — [训练完成][否决] 冻结中心 covariance-only 两档

实际补跑 exact `404,524` 点、center 冻结、只优化 covariance/rotation 与
extinction 的两档训练。弱档 τ MAE `+0.263%`，edge/Gabor 仅改善
`0.106%/0.169%/0.218%`，而 covariance p99 只变化 `0.413%`，视觉幅度不足。

强档 covariance p50/p99 变化 `2.59%/7.05%`，但 τ MAE
`0.062782→0.065600`（`+4.488%`）、τ PSNR 下降 `0.305 dB`，
Gabor energy/phase 恶化 `0.687%/0.892%`，超过 bounded `+0.5%` 容差近
九倍。按 Gate 停止第三档，不生成 PLY、不重烘 transport、不部署 UE。
两档 `recovery_report.json` SHA256 分别为
`2F95E146139CD4F2E754933511FAB1FA06E55DB206C1E5FA1E1484497100AFF4` 和
`90E6C029B1E9139CAF2F1B23AAFA1C1C1AFBC7986B43F3D593A1000B964E391C`，
`numeric_gate_passed` 均为 `false`，stderr 均为空。

### 2026-07-29 — [实现][未启用] Mass-preserving footprint 原型

Gaussian renderer 已加入默认关闭的
`r.GaussianSplatting.FootprintScale=1.0` 原型；投影 footprint 放大时以
`1/scale²` 修正 opacity。质量守恒 self-check 与 AbyssEditor Development
编译通过，但尚未做 live viewport Gate，不计为训练改善，也不默认启用。

随后补充最小回归 `mvp/test_footprint_mass.py`，对
`1.0/1.08/1.15/1.25` 四档验证二维 Gaussian 积分质量守恒，误差在
`1e-12` 内；当前 UE 关闭，等待用户手动打开后做视觉 A/B。

### 2026-07-29 — [更正][未保存] UE 恢复原始 G2

用户发现 TechLab 只剩造型错误的诊断云。UE MCP 最小读回确认关卡内为
ROI-only `35,000` 点和 hybrid `389,524` 点两个诊断 Actor，原 G2 已被原位
替换，并非单纯隐藏。核验插件内 G2 PLY SHA256=
`C4321FCEC040103DF4A615E0598D488F31541F22F2E77007289B4B9EFA564FDF`
后，将 `35,000` 点 Actor 原位加载回 G2，删除另一诊断 Actor，恢复
opacity=`0.6`、power=`0.9`、relight=`0.7`、ambient=`0.01`。最终读回只有
一个 GS Actor：`Compact Transport GS + DGSM | WDAS G2 Sigma038 Aniso115 / 404K`，
point count=`404,524`，位置=`(2600,840,-40)`。本次未保存关卡；用户视觉确认
并明确批准前禁止保存。

### 2026-07-29 — [G13][Identity Gate 通过][正式候选未启动] G2-preserving multi-scale split

首版 multi-scale moment split 从 `412,267` 个 raw B8 parent 重建 exact
`404,524`，因此 split=`0` 也会产生 `7,743` 次合并；其
`split_000000/001000/002000/004000` 及 4K/100-step recovery 不是相对 G2 的
有效增量候选，全部标记为失败诊断、禁止部署。

现有 `build_adaptive_grid_budget.py` 已增加 `--base-npz` 的 exact-G2-preserving
模式：`397,506` 个 singleton 可参与局部 split/merge，`7,018` 个 contracted
records 原样保护。split=`0` 输出与 G2 initializer SHA256 完全一致，均为
`1769870A2FD6C78DDA06F22C9784BC91B09E8D0E0FB22E88B812CAB476974D70`；
质量、全局中心和 covariance error 均为 `0`。3 个最小测试与四脚本
`py_compile` 通过。

当前无 Python 训练进程。新的 base-preserving `1K/2K/4K` 尚未生成或评估，
也没有 PLY、transport 或 UE 部署；下次先同步 ETA，再跑无训练 eval0 Gate，
不接近 G2 的候选不进入 recovery。

### 2026-07-29 — [G14][用户复验通过] G2 光照收口，仅余规则点阵

用户指出 G2 正面受光偏黑。审计确认 compact PLY 的 `j_0..j_5` 按资产局部
`+X/-X/+Y/-Y/+Z/-Z` 存储，而 runtime 曾将 Y/Z angular weights 错配；共享
canonical axis 修复不增加字段、buffer 或 pass。

期间曾令 compact 跳过疑似重复的 DGSM，用户立即确认该版对比度拉不开、背面
压不暗；该假设判负并完整回滚，G2 继续使用原 `bEnableRNGDGSM=true`，其余
opacity=`0.6`、power=`0.9`、relight=`0.7`、ambient=`0.01` 与 PLY 均未改。
回滚后 AbyssEditor 构建、链接、404,524 点加载与
`GaussianSplatting.CompactTransport` NullRHI 测试通过。用户最终确认光照不再
是阻断项；当前唯一剩余视觉问题为 B8 规则点阵，后续只推进 G13。

### 2026-07-29 — [G14][实现完成][待用户复验] SkyAtmosphere 日落光色直连

后续日落 A/B 截图纠正了“Gaussian 已跟随大气光色”的判断：VDB 已明显变为
橙红色，G2 仍近白。只读 TechLab 诊断确认关卡只有一盏 `Light Source`，G2 已
绑定该灯；其 `Atmosphere Sun Light=true`、index=`0`、per-pixel=`false`、保存
intensity=`4.0548029`，因此排除绑错灯和 G2 配置问题。根因是 Gaussian composite
使用组件原始白光后再自行做 `Ground/Outer` heuristic，没有直接消费 UE 原生
VDB/场景光照链路已解析的大气方向光颜色。

最小修复删除该 heuristic：scene-light 模式直接使用
`ResolvedView.DirectionalLightColor × PI` 还原 UE atmosphere-resolved illuminance，
再乘原 `RelightIntensityScale` 与 tint；manual-light 模式保持原光能语义。未修改
G2 PLY、关卡、DGSM、opacity=`0.6`、power=`0.9`、relight=`0.7` 或 ambient=`0.01`。
`AbyssEditor Win64 Development` 构建／链接成功；NullRHI 与 DX12 SM6 offscreen
均加载 exact `404,524` 点并通过 `GaussianSplatting.CompactTransport`，日志无
shader/RDG/GPU crash/fatal。最终日落色相仍需用户在 live viewport 旋转同一方向
光复验；通过前不把 B8 点阵重新标记为唯一视觉阻断项。

### 2026-07-30 — [G15][离线结构路线否决][renderer A/B 待验] B8 点阵消除

完成 jitter、矩平衡 jitter、covariance/warp、split/merge、footprint、screen
blur、时域随机、分层随机采样和 B4 自适应矩分区等路线。B4 adaptive 的
lattice order 从 G2 `0.810900` 降到 `0.002383`，organic05 最低
`0.001269`；但近景对比显示点阵被颗粒／斜纹替代。最佳
organic05+mass05 的 τ PSNR/MAE=`37.4646 dB/0.107946`，仍显著差于
G2 `42.6659 dB/0.060181`；100-step extinction recovery 也无实质追回。
因此不导出、不重烘 transport、不部署、不替换 G2。

渲染链审计确认当前 pass 位于 TSR/TAA 之后；新增默认关闭
`r.GaussianSplatting.PreTSR=0` 供原 G2 临时切到 `BeforeDOF`。另将原硬编码
`1/255` alpha discard 暴露为默认值不变的
`r.GaussianSplatting.AlphaCutoff`。统计显示旧 cutoff 会整颗丢弃约
`8.151%` 的 G2 低密度点，并把中位支持截到约 `2.384σ`。两项均已冷编译通过，
未写配置、未改 G2 PLY／DGSM／灯光／关卡／Actor 参数；待 live viewport 依次
测试 PreTSR、`1/1024` 和 `1/4096`。

### 2026-07-30 — [G18][主线切换][画质收口] S3 312K 标准几何重光照云

纯标准 3DGS 训练证明自适应 densify/prune 的几何轮廓显著优于规则 B8/G2。
官方训练在 15K 得到 `311,993` points，test L1=`0.00240468`、
PSNR=`43.5372 dB`；30K test PSNR 回落到 `42.9078 dB`，因此选 15K。
随后把同源六轴静态 transport 传递到标准几何，改为 shared-opacity 单记录
compact path，并移除 baked DC 的重复光照。当前 UE 实际加载：

`WDAS_S3_StandardGeometry312K_SharedOpacity_DirectJ_NoBakedDC.ply`

该资产为 `311,993 × 64 B`，primitive payload=`19,967,552 B`
(`19.043 MiB`)，SHA256=
`2CC4C78200369389E42A208A6BA2F3EEA6CB042362209B643D7F3A884B8A6607`。
用户确认规则点阵已不再是主问题，方向光、天空色温与体积明暗已达到可继续收口
的状态。

当前剩余问题被重新定义为 silhouette 结构而非像素锯齿：Gaussian 外轮廓由
过多中小团块顶出密集鼓包和凹口，内部硬缝穿到外缘，最外圈 density/opacity
下降过快。目标是只保留较大的云形起伏、内部细节不逐团穿透、边缘连续消散。
现有 2.25 px edge-only premultiplied filter 与 4 px composite AABB padding
在 live viewport 无可辨识变化，因为处理尺度只有 1–2 px；不再继续扩大普通
RGB blur。

训练审计同时发现当前 loss 的直接缺口：coverage 使用 binary-mask BCE，
会把边缘推向 0/1；`T_view` L1 又只乘前景 mask，边缘外侧的连续软尾没有监督。
下一主线从 15K checkpoint 小步 fine-tune，在不增点、不改用户组件参数的前提
下，用连续 `T_view/τ` 覆盖完整边界带并加入多尺度 silhouette loss。只有训练
不足时才增加低分辨率 coverage/τ resolve；不得把内部颜色和光照一起模糊。

性能工作改为 visual Gate 后逐项取证：同机位采集 complete frame、volume pass、
steady/peak working set，并分解 primitive、visible/sort、GSColor、DGSM 与
RDG transient。`19.043 MiB` 只记录为 primitive/PLY payload，不作为相对 SVT
总显存 headline。所有性能改动必须保持当前轮廓、软尾和重光照质量，否则回退。

### 2026-07-30 — [G18][根因收敛][Live Gate] 标准 alpha 对齐与外轮廓专用过滤

连续 `T_view` 教师已改为直接写入 RGBA alpha，并完成独立 held-out 对照。
15K 标准 3DGS 相对同源 VDB 的 8-view mean alpha L1=`0.00469735`，轮廓与
软尾几乎重合；因此停止无目标的 geometry fine-tune。剩余差异主要来自 UE
shared-opacity compact raster 与最终 composite，而不是训练出的标准几何。

光栅审计发现 compact path 在标准 3DGS 的 `+0.3 px²` low-pass 之外又增加
`+3 px²` 方差，并把支持范围从 `3σ` 扩到 `3.5σ`，会令外缘 splat 互相膨胀、
小团块更容易顶穿轮廓。现已仅对 shared-opacity 路径恢复标准 `3σ` 语义并移除
额外方差；未改 PLY、关卡、灯光、DGSM 或用户组件参数。DX12 live shader
重编译完成，当前会话无 shader error、GPU crash 或 fatal。

现有 shared-opacity composite 继续复用原 9-tap edge-only filter，不增加 pass
或采样数。现场 A/B 将采样半径由 `2.25 px` 逐步验证到 `4/6/8 px`，最终暂选
`8 px`，AABB padding 同步为 `9 px`。过滤仍只在低 opacity 外轮廓带混合：
内部高 opacity 团块和重光照细节保持原样，最外圈衰减变宽，中小鼓包／凹口在
中远景合并。半径变化不增加 texture tap 数；新增成本仅是 AABB 外扩的少量
composite 像素，下一步以用户视觉 Gate 决定是否冻结。

### 2026-07-30 — [G19][收口候选][自截图 Gate] 精确 alpha 训练审计与边界 coverage 重建

从 15K checkpoint 使用精确 RGBA alpha 和 17 px 多尺度 silhouette loss 做了
三组不增点 fine-tune。全参数 `w=0.05/500` 将 8-view alpha L1 从
`0.00469735` 降到 `0.00376314`、alpha PSNR 从 `37.14` 提到 `39.08 dB`，
但 compact transport 进入 UE 后中景外轮廓更碎；`w=0.2/1000` 的 alpha L1
反而退到 `0.00684528`，明确过拟合。冻结 geometry 的 opacity-only
`w=0.1/1000` 得到 alpha L1=`0.00377605`，UE 仍未胜过原始 15K。三者均保留
为证据，不晋升主资产、不增加 `311,993` 点预算。

渲染端最终改为边界专用 5×5 Gaussian latent coverage 重建：4 个 8 px
cardinal probe 判定外边界，内部直接跳过；只有边界执行 25 taps，kernel step
为 4 px、半径 8 px、AABB pad 为 9 px。它不新增 pass/RT，也不模糊内部颜色、
阴影或体积层次。5 帧平均的外轮廓 roughness 中景为 GS/SVT=`5.917/4.008`，
远景=`4.354/4.012`；远景进入接近区间，中景仍有差距但小鼓包、凹口和硬边
明显收敛。6 px 半径会退化到中景/远景=`6.190/4.536`，已回退。

最终 editor 回读仍加载原
`WDAS_S3_StandardGeometry312K_SharedOpacity_DirectJ_NoBakedDC.ply`；
opacity=`0.4`、power=`1`、relight=`0.3`、ambient=`0.01`、DGSM density=`0.45`
及 phase 参数全部未变。最终 shader live compile 无 error/fatal。中景/远景
证据为 `evidence/s3_final_coverage8_medium_20260730.png` 和
`evidence/s3_final_coverage8_far_20260730.png`。性能结论仍等待同机位 GPU
profile；当前只确认新路径无新增纹理资源，不能据此宣称 GPU 时间下降。

### 2026-07-31 — [G28][Gate closeout][S3 冻结基线]

本轮收口并通过视觉验收 Gate。验收范围明确为中景/远景静态体积代理：保留较大的云形
起伏，内部允许丰富团块但不让团块逐个穿透外轮廓，最外圈连续衰减，并支持方向光/天光
重光照；近景 Hero 和严格 matched-quality 性能不作承诺。用户确认当前版本可作为后续优化
起点。
...
`311,993` 点、`64 B/point`、`19,967,552 B` payload。构建路径为 VDB teacher → 标准
3DGS 15K 几何 → 12.5% view-boundary/150-step 轮廓候选 → `J^0.4` compact transport →
UE preprocess/count-prefix-scatter/GPU sort/HW Quad/shared-opacity composite/DGSM+phase。

运行时锁定 opacity=`0.4`、power=`1.0`、relight=`0.3`、ambient=`0.01`、DGSM
density=`0.45`，phase/Actor transform 保持原配置；不保存关卡、不新增 pass/RT、不增点。
AlphaCutoff、internal-alpha、1.04× footprint 和全图 blur 分支均已回滚。`19.043 MiB`
仅为资产大小；历史同机位 7DRGS `9.19 ms` 对 SVT U8 `8.43 ms`，性能 Gate 尚未声称胜出。
后续优化必须另建版本并通过同口径回归。

### 2026-07-31 — [G35～G38][项目收尾][视觉、性能与显存闭环]

G28 boundary-morph 之后，用户复看认为原始标准 3DGS 15K 边缘更自然；正式资产回滚为
原始 15K geometry/opacity，并逐 bit 继承当前 `J^0.4` 六轴 transport。G31/G32 通过
原始边缘和双侧结构 coverage 桥接解决蓝底贯穿空槽，G33 加入轮廓切线引导；G34 的稀疏
大步长方向滤波产生纵向梳状笔刷，完整回滚。G35 改用 `5×5 / 25 taps` coverage/depth
joint bilateral，只过滤内部直射光 latent，用户确认效果并冻结。当前 PLY 为
`S3_Original15KGeometry_CurrentGamma04J_AngularSigma05.ply`，`311,993 × 64 B`，
SHA256=`AE7177BF3753E9905C34208A9D46A2647018F55A49FF13581A717BA1040EA0FB`。

G37 只按 `AlphaCutoff=1/255` 保守收缩 HW quad support，保留像素继续走原 conic、opacity、
J、DGSM、coverage 和 G35 composite。同帧中位数由 GS total=`1.343 ms`、HW Raster=
`0.819 ms` 优化到 `1.093/0.5665 ms`，分别降低 `18.6%/30.8%`；SVT feature=
`3.241 ms`。用户自由镜头复验通过，G37 晋升当前 runtime 基线。

G38 使用 Empty/SVT/GS 三张隔离临时地图，在 D3D12、1920×1080、Development `-game`
下各跑 3 个独立冷进程。整个进程专用显存中位数 SVT/GS=`2664.178/2343.980 MiB`，
节省 `320.198 MiB / 12.019%`；这是整个 UE 进程口径。相对 Empty 的单体积净新增 RHI
working set SVT/GS=`305.566/66.476 MiB`，GS 节省 `239.090 MiB / 78.245%`，
约小 `4.597×`。GS 分解为常驻 `19.063 MiB` + transient 净增 `47.414 MiB`；
`2343.980 MiB` 绝不是单个 GS。9/9 进程地图加载成功，3/3 GS 加载 311,993 点，
3/3 SVT 报告 `305.566 MiB`，0 GPU/D3D/shader fatal。

临时地图测后已删除，原 TechLab 重新打开，SVT/GS 均可见，dirty content/map package 为空。
报告为 `PERFORMANCE-VDB-VS-G35-G37-20260731.md` 和
`VRAM-COLD-G37-VS-SVT-20260731.md`；原始显存证据位于
`evidence/memory-20260731-g37-cold-3x/`。当前正结论限定为已签字的中远景静态云与
UE SVT U8，不扩展为 NanoVDB、Shipping、近景 Hero、动画、多光源或通用 VDB 替代。
