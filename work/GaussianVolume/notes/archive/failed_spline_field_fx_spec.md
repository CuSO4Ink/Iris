# [已否决归档] Structured Gaussian Field FX / Spline 开发 Spec

> **状态：失败分支，2026-07-22 已从主干移除。** 该方案未形成相较 Niagara Ribbon、普通 raymarch 或 VDB 的可测量优势，不得作为 GaussianVolume 的当前方向或默认下一步建议。本文仅保留历史实现、验证过程和失败经验。
