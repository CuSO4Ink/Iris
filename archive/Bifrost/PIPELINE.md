...
> **历史设计记录，非当前执行依据。** 本文包含已移出的第三人称跑图、Raymarch 天象、铁磁流体和 Marching Cubes 方案；当前技术范围只以 `TECH-MAP.md` 为准。

> **本文件定位**：技术实现方案（恒星 · 天象 · 铁磁流体 · MCP 量产 · 跑图规模化生产）。**周计划 / 依赖 / 交付基线全在 `ROADMAP.md`**，本文件不复述。
> **前提锁定**：D1=等离子恒星驱动天气 · D2=冷神性 · D3=A 方案·轮廓局部化 · 主机位⑤（S3 段最优截图点）· UE5.8 · FluidFlux 已复刻 · 第三人称跑图 diorama（400m 单向海滩走廊）。
...
> **07-07 修正**：体积云从点缀升到基底（因果链保底 2 支之一）。分色三选一的性质是"未拍板要不要做"（非"决定要做但选哪个"）。**07-07 场景重构**：跑图 diorama 硬约束——基底成片必须能第三人称跑通 400m，跑不通视觉再好也不算保底达成。
...
1. **路径设计先行**：T-P0.0 顶视图手绘 400m 路径线 + 4 段边界 + 4 锚点，是所有布局的锚。
...
400m 海滩单人不可能全手工做完，AI 参与是**可行性前提**而不仅是提效手段。

### 分段生产矩阵

| 段 | LookDev 精力 | AI 参与度 | 人参与度 |
|---|---|---|---|
| S1 远端（0-100m） | 15% | 批量摆位、材质挂载、截图验收 | 段过渡带、远景巨构比例 |
| S2 中景（100-240m） | 15% | 遗迹 kitbash 批量 spawn、材质变体 | 段内节奏、遗迹布局 |
| S3 主锚点区（240-320m） | **60%** | 基础摆位、点缀生成 | **主机位构图精修、LookDev 定稿、铁磁流体/极光美术判断** |
| S4 回望端（320-400m） | 10% | 批量摆位、快扫质量 | 段过渡带、结尾视觉收束 |
...
**1 张环境 Uber 母材质 `M_Env_Uber` + N 张 MI 变体**全部 AI 批量生成（07-07 由"3 张母材质分工"收口为 uber 架构，理由见 `LOG.md`）：
- 母材质内建全部 feature：朝向感知遮罩（`MF_OrientationMask`）、程序化宏观噪声（`MF_MacroVariation`）、细节法线混合、湿度层（`MF_WetnessBlend`）、风化层（`MF_WeatheringOverlay`）、叠加层（`MF_OverlayBlend`，苔藓/锈蚀/盐渐通用）——每层用 Static Switch（`EnableWetness`/`EnableWeathering`/`EnableOverlay`）分组开关，不需要的层零成本关掉。
- 参数全部通用命名（`Base_*`/`Wetness_*`/`Weathering_*`/`Overlay_*`），不绑定具体材质类型；沙/岩石/遗迹金属只是 MI 里不同的参数取值组合（颜色、Roughness、Metallic、各层开关+强度），不是分裂的母材质。
- 已交付 6 张 MI：`MI_Sand_Dry_01`/`MI_Sand_Wet_01`/`MI_Rock_Base_01`/`MI_Rock_Weathered_01`/`MI_RuinMetal_Fresh_01`/`MI_RuinMetal_Oxidized_01`。
...
- 与 `CONCEPT.md`（美术定案）· `D3-KERNEL.md`（恒星实现记录）· `ROADMAP.md`（执行主图）· `RESTART-NOTES.md`（冻结重启条件）互引
